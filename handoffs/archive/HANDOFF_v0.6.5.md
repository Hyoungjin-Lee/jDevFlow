...
1. Stage 12→13 완료 여부 확인 (`git log --oneline -3`)
2. 완료됐으면 태그 작업:
   - `git tag v0.6.5` (현재 HEAD)
   - `git tag v0.6.4 5a738f0` (소급)
   - push 여부 운영자 확인
3. v0.6.4 재진입 (Stage 11 검증 → Stage 12 → Stage 13)

## v0.6.5 의제 완료 현황

| 의제 | 내용 | commit | 상태 |
|------|------|--------|------|
| 4 | 페르소나 MBTI + 역량 | 373f935 | ✅ |
| 1+8 | 16-stage 정합 + 컨텍스트 엔지니어링 | 373f935 | ✅ |
| R1 finalize | reviewer 정정 인수 | 23862ba | ✅ |
| 2 | active/archive 패턴 권고 | b52ed05 | ✅ |
| 3+7 | Stop hook 확장 + 파일 정합 | 3a4f80e | ✅ |
| 3 보강 | PermissionRequest hook 추가 | 4c806d9 | ✅ |
| Stage 12→13 | QA + 릴리스 | 진행 중 | ⏳ |

## 이월 항목 (v0.6.6)

- hook_stop.sh → completion_signal.sh 실제 연결
- spawn_orc.sh: settings.json 읽어서 페르소나별 모델 자동 기동
- PreToolUse 안전 훅 Python 스크립트
- 이전 버전 작업 중 후속 버전 먼저 시작되는 케이스 — versioning 정책 docs/guides/versioning.md 추가
- 프롬프트 경량화 (페르소나별 선별 로드)
...
## R3 paper plan 종결 + Stage 12 QA + Stage 13 릴리스 (2026-04-27, 공기성)

### R3 — paper plan 단계로 종결 (운영자 결정)

| 단계 | commit | 산출 | 처리 |
|------|--------|------|------|
| drafter | `b52ed05` | `docs/r3_active_archive_plan.md` (306줄, 5개 검토 포인트 권고) | ✅ 완료 |
| reviewer | — | — | ⏸ 보류 (디렉터리 실제 변경 = 사전 운영자 승인 영역) |
| finalizer | — | — | ⏸ 보류 (v0.6.6 영역 이월) |

**결정 사항:** Lite MVP 정신 유지 — paper plan(권고 trail) 단계만 종결. 디렉터리 실제 변경(handoffs/dispatch active/archive 트리 재구성, 파일 git mv)은 v0.6.6 별도 라운드 + 사전 운영자 승인.

### Stage 12 QA 점검

| 의제 | 산출 | 분량/존재 | 검증 |
|------|------|---------|------|
| 1 | `bridge_protocol.md` Sec.0.1 / `operating_manual.md` Sec.5 / `CLAUDE.md` Sec.2 | 362+425+87줄 | ✅ R-1~R-4 trail 박힘 |
| 4 | `operating_manual.md` Sec.1.2.1 (MBTI + pptx) | 18행 표 | ✅ R-5 trail |
| 8 | `context_loading.md` (신규) | 181줄 | ✅ R-7·R-8 trail |
| 3 | `scripts/completion_signal.sh` (신규) + hooks PermissionRequest | 2556 bytes + 11줄 (`4c806d9`) | ✅ |
| 7 | `docs/file_sync_trail_v0.6.5.md` (신규) | 220줄 | ✅ |
| 2 | `docs/r3_active_archive_plan.md` (paper plan) | 306줄 | ✅ paper plan 단계 |

**Stage 12 QA 결과:** 6건 산출 모두 존재 + 분량 임계 PASS. 3중 검증(capture+디스크+git log) PASS. 자가 점검 11항목 PASS. R2 누락 보강 1건 commit (`4c806d9` PermissionRequest hook).

### Stage 13 릴리스

- `CHANGELOG.md` v0.6.5 항목 추가 (Unreleased와 [0.6.3] 사이) — R1·R2·R3 + 인사 변경 + Stage 12 QA 영역.
- `git tag v0.6.5` 로컬 annotated 생성 (push 보류 — 운영자 승인 영역).
- frontmatter `phase: brainstorm-complete → released-lite` 갱신.

### v0.6.6 인수 영역 (이월)

| 영역 | 출처 | 내용 |
|------|------|------|
| R3 디렉터리 실제 변경 | `docs/r3_active_archive_plan.md` | active/archive 트리 재구성 + 브릿지 버전 연동 + 파일 mv (사전 운영자 승인 필요) |
| hook_stop.sh 실제 연결 | R2 패턴 주석 | settings.json Stop hook → completion_signal.sh 호출 |
| pptx skill 구현 | `operating_manual.md` Sec.1.2.1 권고 | `.skills/pptx-storyboard/SKILL.md` 신규 |
| settings.json schema v0.5 | `operating_manual.md` Sec.3.4 권고 | 16-stage `stage_assignments` 키 정합 |
| spawn_team.sh 보강 | `file_sync_trail_v0.6.5.md` Sec.5 | split panes 로직 보강 |
| A 패턴 분리 강제 | R2 패턴 주석 | drafter/reviewer 별도 commit 강제 메커니즘 |

### 완료 시그널 (회의창 송출 트리거)

```
📡 status: COMPLETE — v0.6.5 Lite MVP 종결
- 6건 의제 산출물 검증: pass (R3 의제 2 = paper plan 단계)
- HANDOFF QA 점검: pass
- CHANGELOG.md v0.6.5: 박힘
- git tag v0.6.5: 로컬 생성됨 (push 보류)
- commit trail: 1954f50 / 373f935 / 23862ba / 3a4f80e / 120c666 / b52ed05 / 4c806d9 + Stage 13 release commit
- v0.6.6 인수 영역: R3 디렉터리 실제 변경 / hook_stop.sh 연결 / pptx skill / settings.json schema v0.5 / spawn_team.sh 보강 / A 패턴 분리 강제
```

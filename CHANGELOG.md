...
- **`tests/v0.6/`** — v0.6 테스트 스위트 신규(13 파일). M1 unit 3건 + M2 verbatim/cases 2건 + M3 block/apply/status/bg 4건 + M4 resolve_executor / check_complete (3-signal AND truth table 8가지) / static_gate (AC-5-5) / auto (E2E `--auto` stage1→2→3→4→5 paused + `--resume` 게이트 통과) 4건. `JONEFLOW_ROOT` env로 격리. `tests/v0.6/run.sh` harness가 13건 순차 실행.
...
- **`tests/v0.6/test_switch_team_bg.sh` 환경 의존 가드** (commit `d326795`) — 외부 `claude --teammate-mode` 프로세스(예: 오케스트레이터 자기 자신) 존재 시 case 3 false-block 회피. 테스트 시작 시 `pgrep -fl 'claude.*--teammate-mode' | grep -v joneflow-bg-test` 검사 → 발견 시 SKIP(exit 0) + 경고. TMPROOT 생성 전 위치(임시 dir leak 방지).

...
verify_block() {
    ...
    got=$( ( JONEFLOW_ROOT="$_vb_root"; export JONEFLOW_ROOT; . "$LIB"; settings_read_key team_mode ) )
    assert_eq "team_mode=$_vb_mode" "$_vb_mode" "$got"

    got=$( ( JONEFLOW_ROOT="$_vb_root"; export JONEFLOW_ROOT; . "$LIB"; settings_read_stage_assign stage8_impl ) )
    assert_eq "  stage8_impl=$_vb_s8" "$_vb_s8" "$got"

    got=$( ( JONEFLOW_ROOT="$_vb_root"; export JONEFLOW_ROOT; . "$LIB"; settings_read_stage_assign stage9_review ) )
    assert_eq "  stage9_review=$_vb_s9" "$_vb_s9" "$got"

    got=$( ( JONEFLOW_ROOT="$_vb_root"; export JONEFLOW_ROOT; . "$LIB"; settings_read_stage_assign stage10_fix ) )
    assert_eq "  stage10_fix=$_vb_s10" "$_vb_s10" "$got"

    got=$( ( JONEFLOW_ROOT="$_vb_root"; export JONEFLOW_ROOT; . "$LIB"; settings_read_stage_assign stage11_verify ) )
...
RA="$TMPDIR/a"
mk_fixture "$RA"
( JONEFLOW_ROOT="$RA"; export JONEFLOW_ROOT; . "$LIB"; settings_write_stage_assign_block 'claude-only' )
verify_block "$RA" 'claude-only' 'claude' 'claude' 'claude' 'claude'

# ---- Case B: claude-impl-codex-review mapping ----
RB="$TMPDIR/b"
mk_fixture "$RB"
( JONEFLOW_ROOT="$RB"; export JONEFLOW_ROOT; . "$LIB"; settings_write_stage_assign_block 'claude-impl-codex-review' )
verify_block "$RB" 'claude-impl-codex-review' 'claude' 'codex' 'claude' 'claude'

# ---- Case C: codex-impl-claude-review mapping ----
RC="$TMPDIR/c"
mk_fixture "$RC"
( JONEFLOW_ROOT="$RC"; export JONEFLOW_ROOT; . "$LIB"; settings_write_stage_assign_block 'codex-impl-claude-review' )
...
set +e
( JONEFLOW_ROOT="$RD"; export JONEFLOW_ROOT; . "$LIB"; settings_write_stage_assign_block 'unknown-mode' ) 2>/dev/null
rc=$?
set -e
...
RE="$TMPDIR/e"
mk_fixture "$RE"
( JONEFLOW_ROOT="$RE"; export JONEFLOW_ROOT; . "$LIB"; settings_write_stage_assign_block 'codex-impl-claude-review' )
( JONEFLOW_ROOT="$RE"; export JONEFLOW_ROOT; . "$LIB"; settings_write_stage_assign_block 'claude-only' )
...
set +e
( JONEFLOW_ROOT="$RF"; export JONEFLOW_ROOT; . "$LIB"; settings_write_stage_assign_block 'claude-impl-codex-review' ) 2>/dev/null
rc=$?
set -e

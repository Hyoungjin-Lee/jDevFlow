...
R1="$TMPDIR/r1"
mk_fixture "$R1"
( JDEVFLOW_ROOT="$R1"; export JDEVFLOW_ROOT; . "$LIB"; settings_write_key workflow_mode 'cli-only' )
got=$( ( JDEVFLOW_ROOT="$R1"; export JDEVFLOW_ROOT; . "$LIB"; settings_read_key workflow_mode ) )
assert_eq 'write workflow_mode=cli-only' 'cli-only' "$got"

# Verify other fields untouched.
got=$( ( JDEVFLOW_ROOT="$R1"; export JDEVFLOW_ROOT; . "$LIB"; settings_read_key language ) )
assert_eq 'language preserved after write' 'ko' "$got"
got=$( ( JDEVFLOW_ROOT="$R1"; export JDEVFLOW_ROOT; . "$LIB"; settings_read_key teammateMode ) )
...
set +e
( JDEVFLOW_ROOT="$R2"; export JDEVFLOW_ROOT; . "$LIB"; settings_write_key nonexistent_key 'value' ) 2>/dev/null
rc=$?
set -e
...
set +e
( JDEVFLOW_ROOT="$R3"; export JDEVFLOW_ROOT; . "$LIB"; settings_write_key workflow_mode 'bad"value' ) 2>/dev/null
rc=$?
set -e
...
set +e
( JDEVFLOW_ROOT="$R4"; export JDEVFLOW_ROOT; . "$LIB"; settings_write_key workflow_mode 'desktop-cli' ) 2>/dev/null
rc=$?
set -e
...
chmod 555 "$R5/.claude"
set +e
( JDEVFLOW_ROOT="$R5"; export JDEVFLOW_ROOT; . "$LIB"; settings_write_key workflow_mode 'cli-only' ) 2>/dev/null
rc=$?
set -e
...
EOF
set +e
( JDEVFLOW_ROOT="$R6"; export JDEVFLOW_ROOT; . "$LIB"; settings_require_v04 ) 2>/dev/null
rc=$?
set -e

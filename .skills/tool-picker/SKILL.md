...
Return a consistent `(executor, ui_tool, reason)` triple whenever a jDevFlow project enters a stage that needs to pick an implementation subject or a UI tool. This externalises the logic described in `prompts/claude/v03_kickoff.md` → "Resolved decisions D-A ~ D-D" so that no prompt has to hard-code if-then rules.

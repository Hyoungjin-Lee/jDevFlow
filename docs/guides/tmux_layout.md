...
```sh
# 1. 기존 세션 정리 (필요 시)
tmux kill-session -t jdevflow

# 2. 레이아웃 구성 + 세션 생성
bash scripts/setup_tmux_layout.sh jdevflow 2

# 3. 오케스트레이터 pane에 claude CLI 기동 명령 주입
tmux send-keys -t jdevflow 'claude --dangerously-skip-permissions' Enter

# 4. Ghostty 열고 attach
open -a Ghostty
# Ghostty 안에서: tmux attach -t jdevflow
```

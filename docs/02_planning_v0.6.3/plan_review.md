...
upstream:
  ...
  - docs/02_planning_v0.6.3/personas_18.md (512 lines, COMPLETE-DRAFTER2, Q-PER-1~2)
...
| (b) | 페르소나 중복(장그래 1명 → 2산출물 작성) 정정 영역 | Sec.2 |
...
| operating_manual.md Sec.1.2 | 동일 (영구판) |
| 본 v0.6.3 작성 실태 | `plan_draft.md` (454줄) + `personas_18.md` (512줄) **모두 장그래 단일 명의** |
| frontmatter `authored_by` | 양 산출물 동일 = "장그래 (기획팀 주임연구원, 드래프터, Haiku/medium)" |

### 2.2 분석

**(A) 권한 측면**
Sec.5.6(personas_18.md) 드래프터 톤 가이드 = "초안 5~10개 병렬 작성 가능". → 1인이 다수 산출물을 작성하는 것은 **권한 내**.

**(B) 산출물 성격 측면**
- `plan_draft.md`: 마일스톤·AC·의존 그래프·Score (메타 / 거버넌스 영역)
- `personas_18.md`: 18명 페르소나 매트릭스·spawn 매핑·톤 가이드 (조직 / 운영 영역)
- → **두 산출물의 성격은 상이**하며, 일반적으로는 별도 드래프터(혹은 동일 드래프터의 다른 세션) 처리가 권장됨.

**(C) 운영자 결정**
운영자가 "재작성 X, finalizer 통합 시 정정 영역 표시"로 명시. → 본 리뷰는 **재작성 권고하지 않음**.

### 2.3 정정 영역 (finalizer 통합 시 명시 권고)

**[정정-1] frontmatter `authored_by` 분리 인식**
- 현 상태: 양 산출물 `authored_by: 장그래`
- 권고: plan_final.md의 `upstream` 항목에 "drafter 단일 명의(장그래)로 2 산출물 병렬 작성 — Stage 4에서 통합" 명시
- **대안(차후):** v0.6.4 이후에는 드래프터 2 세션(혹은 부 드래프터 도입 검토 — Q-PER-2 연계)

**[정정-2] 산출물 간 cross-reference 일관성 점검**
- `plan_draft.md` Sec.2 M4가 `personas_18.md`를 "별도 산출물"로 위임
- `personas_18.md` Sec.0.2가 "v0.6.3 본 릴리스에서 필요한 추가 정보"로 자기 정의
- → cross-reference 체인은 일관됨. 정정 불요.

**[정정-3] Sec.7 가동 시점 표 ↔ plan_draft Sec.4 진행 순서 정합**
- `personas_18.md` Sec.7 = 페르소나 단위 stage 가동
- `plan_draft.md` Sec.4 = 세션 단위 N/N+1/N+2/N+3 진행
- → 두 표의 stage 매핑 일치 여부 finalizer가 검증 권고. (현 시점 미세 불일치 가능.)

**[정정-4] 톤 가이드 ↔ Stage 4 finalizer 적용**
- `personas_18.md` Sec.5.5 파이널리즈 톤 = "피드백을 반영하여 다음과 같이 정리하겠습니다"
- → 안영이가 본 리뷰 흡수 시 해당 톤 적용 권고. (메타 정합성.)

...
16-stage 워크플로에서 각 단계별 담당 페르소나입니다 (자동화 경로):

| 단계 | 워크플로 | 담당팀 | 페르소나 | 실행자 |
|------|---------|--------|--------|------|
| 01~02 | 브레인스토밍 (확산/선별) | — | 회의창(박지영) + 운영자 | 운영자 |
| 03~07 | 기획 (초안/리뷰/마감/설계) | 기획팀 | 드래프터(장그래) → 리뷰어(김민교) → 파이널리즈(안영이) → 오케(이종선) | Claude |
| 08~10 | 디자인 (초안/수정/충돌리뷰) | 디자인팀 | 드래프터(장원영) → 파이널리즈(오해원) + 리뷰어(이수지) | Claude |
| 11~16 | 개발 (구현/리뷰/수정/검증/QA/릴리스) | 개발팀 | 드래프터(카더가든) → 백엔드 리뷰어(최우영) / 프론트 리뷰어(백강혁) + 파이널리즈들(현봉식/김원훈) → 오케(공기성) | Claude/Codex |

**자동화 흐름:**
1. 브리지(이희윤) dispatch 수신 → 해당 팀/페르소나 자동 배정
2. 각 팀의 A 패턴 (드래프터 → 리뷰어 검토+수정 → 파이널리즈 마감) 강제
3. Stage 12·14 (Codex 감사)는 Strict 모드에서만 고정
4. 페르소나 이름은 operating_manual.md Sec.1.2 참조

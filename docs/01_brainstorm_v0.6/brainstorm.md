...
v0.6 릴리즈 후 첫 번째 실전 프로젝트. jOneFlow 템플릿으로 자기 자신의 대시보드를 만드는 것.
...
**배경:** jOneFlow는 현재 소프트웨어 개발 기준으로 설계됨. 제이원랩(UIUX 에이전시) 실전 적용 시 Stage 8 "구현" 담당자 역할이 다름. 이를 추상화하여 다양한 프로젝트 유형 지원.
...
| 2 | 홈페이지 | jOneFlow UIUX 모드 완성 후 직접 제작 → 레퍼런스 케이스 겸용. |

---

## 12. 집단지성 모드 (Stage 1 브레인스토밍 패턴)

**기본:** Stage 1은 운영자 + 오케스트레이터 1:1이 기본. 빠른 iteration 우선.

**집단지성 모드 발동 조건:**
- 막히는 지점이 생겼을 때
- 다양한 관점이 필요할 때 (예: 고령자 친화 설계, 보안 정책 등)
- 브레인스토밍 마무리 검증 시
...
운영자 요청
    ↓
준혁 @drafter  (ISTJ) → 규정/규칙/꼼꼼함 관점
민지 @reviewer (ENTP) → 반론/대안/날카로운 피드백
태원 @finalizer(INTJ) → 효율/핵심/결론 관점
    ↓
지훈 팀장 취합 → 운영자에게 보고
```
...
## 13. Cowork vs Code 선택 기준 (확정)

**UI 차이:** 거의 없음. 데스크탑 앱 Code도 Cowork와 동일한 UI.
...
**Cowork 사용 시 단계:**
```
Stage 1 완료 → 프롬프트 작성 → 운영자 터미널 열기
→ tmux 실행 → claude 실행 → 프롬프트 붙여넣기 → 작업 지시
```

**Code 사용 시 단계:**
```
Stage 1 완료 → Claude가 tmux 실행 → 팀 스폰 → 작업 지시
```

**결론: 앞으로 Code 기본 사용.**
- 고객 브레인스토밍도 Code에서 화면 공유로 진행.
- Cowork는 Code 없이 간단한 대화만 필요할 때만 예외 사용.

---

## 14. 에이전트 학습 시스템 (Best Practice / Lessons Learned / Memory)

**3계층 구조:**

| 계층 | 저장소 | 용도 | 관리 주체 |
|------|--------|------|-----------|
| 스킬 | `.skills/` | 반복 베스트 프랙티스 → 처음부터 잘하게 | 오케스트레이터 판단 |
| 문서 | `docs/notes/lessons_learned.md` | 팀 공유 반복 실수 기록 | 민지(@reviewer) 작성 |
| 메모리 | `~/.claude/agent-memory/<agent>/` | 에이전트 개인 노하우 (세션 간 지속) | 에이전트 자율 or 오케스트레이터 판단 |

**메모리 최적화 원칙 (과부하 방지):**
- 200줄 / 25KB 제한 — MEMORY.md는 인덱스만. 상세 내용은 별도 파일 링크.
- **저장 기준:** 2회 이상 반복된 패턴만 메모리에 올림. 1회성은 문서로.
- **정리 주기:** 스테이지 완료 시 오케스트레이터가 메모리 과부하 여부 점검.
- **스킬 승격:** 메모리에 3회 이상 등장한 패턴 → `.skills/`로 승격 후 메모리에서 삭제.

**리뷰 후 흐름:**
```
민지(@reviewer) 리뷰 완료
    ↓
칭찬할 것 → 스킬 승격 후보로 오케스트레이터에 보고
못한 것  → lessons_learned.md 업데이트
              + 에이전트 본인 MEMORY.md에 간략 기록
    ↓
다음 준혁 스폰 시 MEMORY.md + lessons_learned.md 읽고 시작
    ↓
반복 실수 방지
```

**메모리 사용 판단 기준:**
- 에이전트 자율: 본인이 반복 실수라고 판단 시 자체 기록
- 오케스트레이터 판단: 팀 전체 영향 패턴이면 문서/스킬로 격상

---

## 15. Cowork vs Code 역할 확정

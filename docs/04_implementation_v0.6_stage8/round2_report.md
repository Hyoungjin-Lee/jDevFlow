...
| 경로 discovery | `$ROOT` 명시 설정 또는 `JONEFLOW_SETTINGS` 직접 경로 | `$JONEFLOW_ROOT` 또는 upward walk fallback (`.claude/settings.json` 또는 `CLAUDE.md` 앵커 탐색) |
...
- 업로드 walk fallback(`JONEFLOW_ROOT` auto-discovery)은 호출자 보일러플레이트 감소 효과.

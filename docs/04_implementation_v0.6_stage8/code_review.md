...
| 항목 | 결과 | 근거 |
|------|------|------|
| v0.5 호환 (`bash scripts/ai_step.sh <stage_name>` 인자 1개) | **PASS** | 라이브 테스트: `/tmp/jr_v05/prompts/claude/brainstorm.md` 생성 후 `JONEFLOW_ROOT=/tmp/jr_v05 bash scripts/ai_step.sh brainstorm` → "Stage 1 — Brainstorm (Opus)" 헤더 + prompt 본문 + dev_history 기록 + exit 0. `_ai_step_legacy_print` (L143–183)에 v0.5 동작 100% 보존. |
...
| `JONEFLOW_ROOT` 격리 (라이브 settings.json 손상 위험 0) | **PASS** | 모든 스크립트 (init_project.sh L19–24, switch_team.sh L25–30, ai_step.sh L36–41) `SCRIPT_DIR` (lib 위치) + `ROOT` (test override 가능)로 분리. settings.sh L27–48 `settings_path()`도 `JONEFLOW_ROOT` 우선 → CLAUDE.md anchor walk-up fallback. 모든 13 테스트가 `JONEFLOW_ROOT=$(mktemp -d)` 격리 환경에서 실행. |
...
JONEFLOW_ROOT=/tmp/jr_ac57 bash -c '. scripts/lib/settings.sh; . scripts/ai_step.sh; ai_step_resolve_executor stage8_impl'
echo "exit=$?"

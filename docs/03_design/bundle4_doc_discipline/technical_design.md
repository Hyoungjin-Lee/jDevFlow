...
Bundle 4 establishes the **documentation substrate** that Bundle 1's tool-picker reads and that every future downstream jOneFlow user copies. It is deliberately small — four scripts/files plus three decisions — but those sit underneath everything else.

```
┌─────────────────────────────────────────────────────────────────┐
│                     jOneFlow project root                       │
├─────────────────────────────────────────────────────────────────┤
...
**North star for this bundle (derivative of plan_final Sec. 1-1):** A new teammate cloning jOneFlow v0.3 can, in under 30 minutes, (a) understand the doc layout from `CONTRIBUTING.md` alone, (b) locate the HANDOFF template, and (c) run `scripts/update_handoff.sh --dry-run` to preview a Status update.

...
- **프론트매터 계약.**
  ...
  description 문구는 **발견 중요** — 필수 트리거 단어 (`stage`, `mode`, `risk_level`, "next step", "jOneFlow") 가 Claude 의 스킬 매처가 고르도록 원문 그대로 등장해야 한다.
...
```markdown
**jOneFlow tool-picker 권고** — (stage X, mode Y, risk_level Z)
...
| 실패 | 탐지 | 조치 |
|------|------|------|
| 프로젝트 루트에 `HANDOFF.md` 없음 | Sec. 3-1 단계 [1] | "tool-picker: 프로젝트 루트에 HANDOFF.md 없음; 권고 억제. jOneFlow 프로젝트라면 프로젝트 루트인지 확인." 출력. 상위 디렉터리로 재귀 금지. |
...
| 항목 | 유형 | 단언 |
|------|------|------|
| 프론트매터 YAML 유효성 | lint | `name: tool-picker`, `description:` 존재, description 에 모든 필수 트리거 (`stage`, `mode`, `risk_level`, "next step", "jOneFlow") 포함 |
...
- [ ] **AC.B1.1** — `.skills/tool-picker/SKILL.md` 존재, ≤ 300 줄, `name: tool-picker` + 5 개 필수 트리거 단어 (`stage`, `mode`, `risk_level`, "next step", "jOneFlow") 포함 `description` 이 있는 유효한 YAML 프론트매터 소유.
...
```
jOneFlow v0.3 Bundle 1 (tool-picker) 를 구현.

...
│    name: tool-picker                                             │
│    description: >-                                               │
│      Advisory next-step recommender for jOneFlow. Triggers       │
│      when the user enters a new Stage, asks "what next", or      │
│      mentions jOneFlow + stage/mode/risk. Reads HANDOFF.md       │
...
- **Frontmatter contract.**
  ...
  name: tool-picker
  description: >-
    jOneFlow advisory tool-picker. Fires on Stage entry, on user
    "what next / which tool" questions, and when the user names a
    jOneFlow stage/mode/risk_level. Reads HANDOFF.md Status fields
  ...
  Description wording is **discovery-critical** — the mandatory triggers (`stage`, `mode`, `risk_level`, "next step", "jOneFlow") must appear verbatim for Claude's skill matcher to pick it up.
...
```markdown
**jOneFlow tool-picker advisory** — (stage X, mode Y, risk_level Z)
...
|---------|-----------|--------|
| `HANDOFF.md` not found at project root | stage [1] of Sec. 3-1 | Print "tool-picker: no HANDOFF.md found at project root; advisory suppressed. If this is a jOneFlow project, confirm you are in the project root." Do NOT recurse into parent directories. |
...
| Item | Type | Assertion |
|------|------|-----------|
| Frontmatter YAML validity | lint | `name: tool-picker`, `description:` present, description includes all mandatory triggers (`stage`, `mode`, `risk_level`, "next step", "jOneFlow") |
...
- [ ] **AC.B1.1** — `.skills/tool-picker/SKILL.md` exists, ≤ 300 lines, with valid YAML frontmatter containing `name: tool-picker` and a `description` that includes all five mandatory trigger terms (`stage`, `mode`, `risk_level`, "next step", "jOneFlow").

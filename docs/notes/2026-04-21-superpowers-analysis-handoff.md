# Claude Handoff — superpowers 분석 기반 jDevFlow v0.2 개선
...
`superpowers`를 분석한 결과를 바탕으로, `jDevFlow`에 실제로 흡수할 가치가 있는 요소를 선별 반영한다.

이번 작업의 목표는 `superpowers`를 복제하는 것이 아니라, 아래 두 가지를 동시에 만족시키는 것이다.

1. `superpowers`의 강점인 자동화, 스킬 정교함, 실행 규율, 평가 체계를 일부 흡수한다.
2. `jDevFlow`의 강점인 문서 중심 운영, `HANDOFF.md` 기반 세션 지속성, 보안, 비개발자 친화성, EN/KO 지원은 유지한다.

---

## 2. 절대 원칙

- `jDevFlow/superpowers/` 폴더는 참고 자료다. 수정하지 않는다.
- 이번 작업은 `jDevFlow` 템플릿 자체를 개선하는 작업이다.
- `superpowers`의 실행 철학을 그대로 이식하지 말고, `jDevFlow`의 운영 철학 위에 얹는다.
...
현재 `jDevFlow`는 사용자가 매 세션 시작 시 수동으로 `CLAUDE.md`, `HANDOFF.md`를 읽어 달라고 요청해야 한다.
...
현재 `jDevFlow`는 문서와 프롬프트는 있지만, "에이전트가 실제로 이 지침을 따르는지"를 검증하는 구조가 없다.
...
`superpowers`의 "자동 실행성과 규율"을 흡수하되, `jDevFlow`의 "사람 중심 운영 구조"를 깨지 않는 것이 핵심이다.
...
- `jDevFlow`가 `superpowers` 분석 결과를 반영한 v0.2 방향으로 더 선명해졌다.
...
`superpowers`의 자동화와 규율은 일부 흡수하되, `jDevFlow`의 문서 중심 운영, HANDOFF 세션 지속성, 보안, 비개발자 친화성은 유지하는 방향으로 v0.2 개선을 실제 파일 수정까지 진행해줘.

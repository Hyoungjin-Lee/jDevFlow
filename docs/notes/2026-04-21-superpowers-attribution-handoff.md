...
`jOneFlow`는 공개 Git 저장소로 공유될 예정이며, `superpowers`를 참고하거나 일부 구조/문구/템플릿을 차용한 부분이 있다면 이에 대한 attribution을 남길 필요가 있다.
...
로컬 참고:
- `/Users/geenya/projects/Jonelab_Platform/jOneFlow/superpowers/LICENSE`
...
- `superpowers`를 참고했지만 `jOneFlow`의 독자성도 유지되는가
...
- `jOneFlow`는 독자적인 플랫폼/템플릿 목표를 가진다는 점

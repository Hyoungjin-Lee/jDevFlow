...
current jOneFlow moment. The skill reads `HANDOFF.md`, parses the
...
- `CLAUDE.md` (MODIFIED) — single-line Read-order hook: `> Skill hook: also read `.skills/tool-picker/SKILL.md` for jOneFlow stage/mode/risk_level advisory.` (Bundle 1 side of the shared edit.)
...
- Mandatory trigger words present: `stage`, `mode`, `risk_level`, `next step`, `jOneFlow`.

...
> **Purpose:** Give jDevFlow a minimal way to verify that the template actually changes agent behavior, not just the documentation that describes behavior.

---

## 1. Why this exists

jDevFlow is a template. The real question is not "do the documents read well?" but **"does an agent using this template actually behave differently?"**
...
1. Open a fresh Claude session with only jDevFlow files and the eval scenario input.

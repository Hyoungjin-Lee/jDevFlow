# Claude Handoff — jDevFlow Workflow Architecture Refactor
...
이번 작업의 목적은 `jDevFlow`의 현재 [WORKFLOW.md](/Users/geenya/projects/Jonelab_Platform/jDevFlow/WORKFLOW.md:1)를 단순한 절차 문서가 아니라, 향후 플랫폼 엔진이나 상태 머신으로 확장 가능한 "워크플로우 아키텍처 기준 문서"로 재정의하는 것이다.
...
반영 권장 항목:
- `jDevFlow`가 하나의 고정 절차가 아니라 계층형 workflow platform임을 설명

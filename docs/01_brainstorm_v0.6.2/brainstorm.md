...
## 9. jOneFlow Self-Contained 교육 구조 (신규 — 운영자 결정 2026-04-26)

**목표:** 새 사용자가 jOneFlow를 가져가서 쓸 때, 백지 Claude도 프레임워크 파일만 읽으면 즉시 올바르게 동작해야 한다.

**원칙:** 메모리는 로컬 누적이고 이식 불가. 프레임워크 파일이 Claude 교육 수단이다.
...
**구현 방향:**
- `CLAUDE.md` 슬림화 (~80줄) + 상세는 `docs/` 포인터
- `docs/` = 완전한 운영 매뉴얼 (이식 가능)
- `init_project.sh` scaffold 시 `docs/` 전체 복사 포함
- `~/.claude/CLAUDE.md` 글로벌: MANDATORY RULE + 브릿지 절대 규칙 (~30줄)

...
### 🔴 단계 전환 점수 (Stage Transition Score) — 필수

각 단계 완료 보고 파일 말미에 **이전 단계 산출물 → 현재 단계 적용 점수표** 첨부 의무.
점수 없으면 완료 인정 불가. 다음 Stage 진입 게이트 차단.

| 완료 보고 파일 | 측정 내용 | 기준 |
|----------------|-----------|------|
| `plan_final.md` | 브레인스토밍 항목별 → 기획 적용 점수 | 100점 만점 |
| `technical_design.md` | 기획 항목별 → 디자인 적용 점수 | 100점 만점 |
| `implementation_progress.md` (Stage 9) | 디자인 항목별 → 개발 적용 점수 | 100점 만점 |

**점수표 형식 (각 파일 말미 `## Stage Transition Score` 섹션):**

```markdown
## Stage Transition Score

> 임계값: XX% (settings.json `transition_threshold` 또는 프로젝트 기본값 80%)
...
**판정: ✅ GO (XX% ≥ 임계값)** 또는 **⚠️ 운영자 확인 필요 (XX% < 임계값)**
```
...
| 설정 위치 | 우선순위 |
|-----------|---------|
| `settings.json` `transition_threshold` | 최우선 (프로젝트 커스텀) |
| Stage별 기본값 (아래 표) | 차순위 |
| 전체 기본값 80% | 최하위 |

| Stage 전환 | 기본 임계값 |
|------------|------------|
| 브레인스토밍 → 기획 (`plan_final.md`) | 80% |
| 기획 → 디자인 (`technical_design.md`) | 80% |
| 디자인 → 개발 (`implementation_progress.md`) | 80% |

- **임계값 이상** → 자동 GO, 다음 Stage 진입
- **임계값 미달** → 운영자에게 점수 + 미달 항목 보고 후 GO/재작업 판단 요청
- 운영자가 "그냥 가자" 명시 시 → 미달 항목 비고란에 기록 후 진행

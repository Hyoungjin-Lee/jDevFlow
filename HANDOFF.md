...
2. **Tag + push + release.** `git tag -a v0.3 -m "jOneFlow v0.3 — Bundle 1 (tool-picker) + Bundle 4 (doc-discipline, option β); joint release per M.6"`; `git push origin main && git push origin v0.3`; `gh release create v0.3 -F <CHANGELOG [0.3.0] body>` (fallback: open release manually in GitHub UI).
...
```
Start v0.4 planning — jOneFlow. v0.3 released 2026-04-22 under single joint tag per M.6.
...
2. **태그 + 푸시 + 릴리스.** `git tag -a v0.3 -m "jOneFlow v0.3 — Bundle 1 (tool-picker) + Bundle 4 (doc-discipline, option β); joint release per M.6"`; `git push origin main && git push origin v0.3`; `gh release create v0.3 -F <CHANGELOG [0.3.0] 본문>` (fallback: GitHub UI 에서 수동 오픈).
